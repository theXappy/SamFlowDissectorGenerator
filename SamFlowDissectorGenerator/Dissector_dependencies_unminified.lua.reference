-- Minifying to the final version was done using this service:
...
function base64_encode(data)
    ...
    return ((data:gsub('.', function(x) 
        local r,Q='',x:byte()
        for i=8,1,-1 do r=r..(Q%2^i-Q%2^(i-1)>0 and '1' or '0') end
        return r;
...
local function padByteString(subtree,data)
	local dataLength = #data
	subtree:add("===== padByteString ==== ");
	subtree:add("data " .. data);
	subtree:add("dataLength " .. dataLength);
	...
	subtree:add("prefix " .. prefix);
	subtree:add("data " .. data);
	data = prefix .. data
	subtree:add("combined data " .. data);
...
function public.encryptString(subtree,key, data, modeFunction, iv)
	if iv then
		local ivCopy = {}
...
-- modefunction - function for cipher mode to use
--
function public.decryptString(subtree,key, data, modeFunction, iv)
...
local function pwToKey(subtree,password, keyLength, iv)
	...
	password = ciphermode.encryptString(subtree,pwBytes, password, ciphermode.encryptCBC, iv)

	-- SAM FLOW DISSECTOR: This line was added to the aes.lua impl
	password = base64_encode(password)
	-- SAM FLOW DISSECTOR: END
...
--
function encrypt(subtree,password, data, keyLength, mode, iv)
	local mode = mode or CBCMODE
	local keyLength = keyLength or AES128

	local key = pwToKey(subtree,password, keyLength, iv)
	...
	elseif mode == CBCMODE then
		return ciphermode.encryptString(subtree,key, paddedData, ciphermode.encryptCBC, iv)
...
-- mode and keyLength must be the same for encryption and decryption.
--
function decrypt(subtree,password, data, keyLength, mode, iv)
	local mode = mode or CBCMODE
	local keyLength = keyLength or AES128
	...
	elseif mode == CBCMODE then
		plain = ciphermode.decryptString(subtree,key, data, ciphermode.decryptCBC, iv)
	elseif mode == OFBMODE then
